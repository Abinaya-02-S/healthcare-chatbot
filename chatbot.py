...
def chatbot():
    ...
    # ---------------- Doctor & Telemedicine Guidance ----------------
    doctor = disease_doctor.get(disease, "General Physician")

    print(f"\n👨‍⚕️ Recommended Doctor Type: {doctor}")

    if doctor in doctor_contact:
        print(f"📞 Doctor Contact: {doctor_contact[doctor]}")

    print("\n📲 Telemedicine Support:")
    for service in telemedicine_services:
        print(service)

    print(f"🔎 Confidence: {confidence}%")
    with open("patient_logs.csv", "a", newline="") as f:
        writer = csv.writer(f)
        writer.writerow([age, gender, symptoms_list, disease, confidence])
